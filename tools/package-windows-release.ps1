...
foreach ($PathToClean in @($Stage, $NsisDirectory)) {
  $ResolvedPath = [System.IO.Path]::GetFullPath($PathToClean)
  $AllowedRoot = if ($ResolvedPath.StartsWith([System.IO.Path]::GetFullPath($DistRoot), [System.StringComparison]::OrdinalIgnoreCase)) {
    [System.IO.Path]::GetFullPath($DistRoot)
  } else {
    [System.IO.Path]::GetFullPath($TargetRelease)
  }
