...
# Current Implementation Notes - Save Version 41
...
The release channel is **Office Aquarium Public Beta 0.9**. Application,
package, Tauri, Cargo, and Cargo-lock versions are kept at `0.9.0`; the
save schema is version `41`.
...
-   Normal UI reports what the organization knows or has reported. AI
    ...
    unreported blockers.
-   Social AI model 4 records source-backed conflict and repair, bounded
    directional memory, grounded visible conversations, slow culture
    evidence, derived informal groups and team chemistry, and formal or
    informal leadership evidence.
-   Conversation Presence extends those grounded events into deterministic
    4-5 turn exchanges with personality-specific wording, shared-memory
    references, pair-level anti-repetition, approach and goodbye phases,
    natural spacing, small gestures, and a return to prior activity.
-   Social systems may influence voluntary interaction, reporting
    interpretation, and bounded emotional recommendations. They must
    not directly change work output, project progress, hiring,
    customers, or Institutional Learning.
...
-   The gear menu opens a complete in-game Simulation Handbook. Handbook
    content and behavior live in `src/ui/simulation-handbook.js`, responsive
    styling lives in `src/ui/simulation-handbook.css`, and shared navigation
    defaults live in `src/core/constants.js`.
-   Production persistence keeps a current save and a validated
    last-known-good backup. Corrupt, missing, migration-required,
    storage-unavailable, and quota states must remain distinguishable and
    player-facing recovery must never silently start a new company.
-   Fatal simulation stages pause safely, record a player-safe stage,
    attempt an emergency save, and show recovery choices instead of looking
    like a frozen office.
-   Every push runs the complete local regression suite and clean web-package
    smoke. Long-run strategy validation and release-candidate packaging use
    the separate workflows under `.github/workflows/`.
-   Public packages must be versioned, hashed, free of private specifications
    and developer output, and accompanied by the release guide, release notes,
...
10. Employees form professional relationships through familiarity, shared experiences, and derived trust/respect/comfort/friction.
...
## Version 39 social memory, conversations, and organization

Version 39 completes the source-backed social architecture. Real workplace events can now create professional conflict, later repair attempts, directional memories, and contextual emotional recall. Memories remain bounded, age in simulated time, compress deterministically, and retain their source events.

The live office can show short grounded conversations for current work, help, blockers, meetings, mentoring, recognition, conflict, repair, company news, and celebration. Ordinary dialogue may be overheard at reduced confidence when distance, room, attention, privacy, and volume allow it. Confidential dialogue is never shown or overheard.

Repeated evidence gradually shapes organizational culture, informal groups, team chemistry, bridge employees, and formal or informal leadership credibility. A People & Culture report makes supported trends visible without exposing raw hidden relationship scores. These systems remain isolated from direct work, project, customer, hiring, and Institutional Learning changes.

Save version 39 also bounds the long-run social state used by larger workforces. Relationship evidence keeps compact summaries while recent detail is retained only where it remains relevant, transient diagnostics stay out of production saves, and repeated save keys use a backward-compatible compact envelope. Older plain JSON saves remain readable. Regression coverage enforces a release storage budget for a mature 29-employee organization with every possible employee relationship represented.

## Version 40 conversation and presence

Version 40 extends the existing grounded dialogue system rather than replacing it. A real simulation event creates a conversation opportunity; eligible employees approach, greet, discuss the source-backed topic, reply, optionally follow up, say goodbye, and resume their latest activity.

Every one of the 15 conversation categories has 40 deterministic templates built from separate intent and personality wording. Pair-level history favors unused templates, topics, categories, and partners. Real shared memories may be referenced, but generic dialogue cannot invent a task, blocker, meeting, outcome, or future promise.

Participants use smooth approach and resume movement, face one another with natural spacing, and continue small deterministic gestures while speaking or listening. Read time scales with line length, replies have pauses, critical work rejects irrelevant interruptions, and a work transition that occurs during a visible conversation is shown immediately after the goodbye. Conversation, movement, trigger, and anti-repeat state survive save/load and deterministic replay.

Help exchanges and blocker-resolution exchanges retain their actual source and use different dialogue categories. Onboarding mentorship waits for real mentor/new-hire co-presence. A later apology or clarification can arise autonomously from an unresolved conflict only when the relationship, personalities, culture, and workplace safety support a credible attempt.

Task-completion and department-return conversations are presentation opportunities only unless a separate real social outcome exists. Displaying a bubble does not itself change relationships, emotions, work, projects, or Institutional Learning.

## Version 38 causal learning and message integrity

Version 38 strengthens Institutional Learning by requiring meaningful, attributable evidence before lessons influence behavior. Learning episodes track domains, attribution quality, intervention/comparison context, review windows, failure cause, and later outcomes. Short observations, displayed messages, policy countdowns, or transition completion alone should not validate a lesson.
