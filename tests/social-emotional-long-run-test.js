...
async function main() {
  ...
  const result = await page.evaluate(() => {
    ...
    const positiveWorkEvidencePairs = relationships.filter(record => {
      const counts = record.relationshipInputs?.counts || {};
      const positiveSharedEvidence = (counts.directHelp || 0) + (counts.success || 0) + (counts.mentoring || 0) + (counts.recognition || 0);
      return positiveSharedEvidence > 0;
    }).length;
    const professionalFrictionEvidencePairs = relationships.filter(record => {
      const rel = record.interpretation || {};
      const counts = record.relationshipInputs?.counts || {};
      const frictionEvidence = (counts.deadline || 0) + (counts.failure || 0) + (counts.interruptions || 0)
        + (counts.conflict || 0) + (counts.trustDamagingConflict || 0);
      return (rel.professionalFriction || 0) >= relationshipBands.materialFriction && frictionEvidence > 0;
    }).length;
    ...
    assert(positiveWorkEvidencePairs > 0, "A natural long run should create positive relationship evidence from actual shared work");
    assert(professionalFrictionEvidencePairs > 0, "A natural long run should derive material professional friction from actual shared-work pressure");
    ...
    return {
      ...
      positiveWorkEvidencePairs,
      professionalFrictionEvidencePairs,
