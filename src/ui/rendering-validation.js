...
function averageRelationship(e){
  ...
  return others.length?others.reduce((sum,o)=>sum+relationshipPreferenceScore(e,o),0)/others.length:0;
}
...
function developerValidationHtml(e,scoreLines,cooldowns,strongestMemory,ceo){
  ...
  const migration=company.socialAIMigrationReport||{};
  const ownershipDebug=`Work decisions: ${e.decisionTrace?.ownerSystem||"unowned"}<br>Work outcomes: ${(company.actionOutcomes||[])[0]?.ownerSystem||"none"}<br>Social preferences: ${(company.socialPreferenceHistory||[])[0]?.ownerSystem||"none"}<br>Social experiences: ${Object.values(company.socialRelationships||{}).flatMap(r=>r.recentExperiences||[])[0]?.ownerSystem||"none"}<br>Emotional state writes: ${(company.emotionalTraces||[])[0]?.ownerSystem||"none"}<br>Institutional evidence: ${(company.learningEvidence||[])[0]?.ownerSystem||"none"}<br><br>Social AI model ${company.socialAIModelVersion}; migration ${migration.fromVersion??"n/a"} -> ${migration.toVersion??"n/a"}; relationships ${migration.relationshipsMigrated??0}; legacy records removed ${migration.legacyRecordsRemoved??0}; experiences preserved ${migration.experiencesPreserved??0}; RNG ${migration.rngStateBefore??"n/a"} -> ${migration.rngStateAfter??"n/a"}`;
...
function employeePersonalityDebugHtml(e){
  ...
  const profile=e.emotionalProfile||{};
  const home=e.lastEmotionalHomeostasis||{};
