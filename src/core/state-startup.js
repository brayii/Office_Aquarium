const SAVE_KEY=OFFICE_AQUARIUM_CONSTANTS.storage.saveKey;
const SAVE_VERSION=OFFICE_AQUARIUM_CONSTANTS.storage.saveVersion;
const AI_SYSTEM_OWNERS=OFFICE_AQUARIUM_CONSTANTS.aiOwners;
const SCORE_DEFAULTS=OFFICE_AQUARIUM_CONSTANTS.defaults;
const SOCIAL_RULES=OFFICE_AQUARIUM_CONSTANTS.social;
const TIME_RULES=OFFICE_AQUARIUM_CONSTANTS.time;
...
Object.assign(initialCompany,{
  ...
  lastInformationalEscalationDay:OFFICE_AQUARIUM_CONSTANTS.time.neverDay,
  lastDecisionEscalationDay:OFFICE_AQUARIUM_CONSTANTS.time.neverDay,
  decisionEscalationFingerprints:{},
  lastHiringRequestMemoDay:OFFICE_AQUARIUM_CONSTANTS.time.neverDay,
  saveHealth:{status:"ready",lastSuccessfulDay:null,lastSuccessfulSize:0,backupAvailable:false,errorCode:null},
  runtimeFailure:null
});
let company,employees,timer=null,debugMode=false,selectedEmployeeId=null,validationMode=false,currentSimulationContext=null,lastValidationReport=null,lastInboxSoundCount=null,musicUnavailable=false,alertsUnavailable=false,pausedBeforeResetPrompt=false,socialAIValidatedCompany=null,handbookOpenedFromStartup=false,lastLoadFailure=null;
...
function personalitySeedForEmployee(eOrId,role="",salt=""){
  ...
  return hashText32(`${SAVE_VERSION}|${company?.randomState??OFFICE_AQUARIUM_CONSTANTS.determinism.defaultRandomState}|${id}|${name}|${role}|${salt}`);
}
...
function defaultEmotionalState(profile={}){
  return {
    socialBattery:Math.round(clamp(SCORE_DEFAULTS.healthyScore+(profile.sociability||0)*18-(profile.structureNeed||0)*8,15,95)),
    frustration:20,
    belonging:Math.round(clamp(SCORE_DEFAULTS.healthyScore+(profile.empathy||0)*10+(profile.collaboration||0)*8,20,95)),
    recognitionSatisfaction:SCORE_DEFAULTS.healthyScore,
...
function defaultEmotionalProfile(e={},profile=e.personality||{}){
  const seed=Number.isFinite(e.personalitySeed)?e.personalitySeed:personalitySeedForEmployee(e,e.role,"emotional-profile");
  const rand=personalityRandom(hashText32(`${seed}|emotional-profile|${e.id??"new"}`));
  const moraleBaseline=clamp(60+(profile.resilience||0)*6+(profile.sociability||0)*4+(profile.empathy||0)*3+(rand()-.5)*14,45,75);
  const stressBaseline=clamp(30-(profile.resilience||0)*7+(profile.structureNeed||0)*4-(profile.adaptability||0)*3+(rand()-.5)*12,15,45);
  const moraleRecoveryRate=clamp(.018+(profile.resilience||0)*.004+(profile.adaptability||0)*.003+(rand()-.5)*.006,.012,.035);
  const stressRecoveryRate=clamp(.024+(profile.resilience||0)*.006-(profile.structureNeed||0)*.002+(rand()-.5)*.008,.014,.045);
  return {
    moraleBaseline:Number(moraleBaseline.toFixed(3)),
    stressBaseline:Number(stressBaseline.toFixed(3)),
    moraleRecoveryRate:Number(moraleRecoveryRate.toFixed(4)),
    stressRecoveryRate:Number(stressRecoveryRate.toFixed(4))
  };
}
function ensureEmployeeEmotionalProfile(e,{force=false}={}){
  if(!e)return null;
  const existing=e.emotionalProfile&&typeof e.emotionalProfile==="object"?e.emotionalProfile:null;
  const generated=defaultEmotionalProfile(e,e.personality||{});
  e.emotionalProfile={
    moraleBaseline:Number.isFinite(existing?.moraleBaseline)&&!force?clamp(existing.moraleBaseline,45,75):generated.moraleBaseline,
    stressBaseline:Number.isFinite(existing?.stressBaseline)&&!force?clamp(existing.stressBaseline,15,45):generated.stressBaseline,
    moraleRecoveryRate:Number.isFinite(existing?.moraleRecoveryRate)&&!force?clamp(existing.moraleRecoveryRate,.01,.05):generated.moraleRecoveryRate,
    stressRecoveryRate:Number.isFinite(existing?.stressRecoveryRate)&&!force?clamp(existing.stressRecoveryRate,.01,.06):generated.stressRecoveryRate
  };
  return e.emotionalProfile;
}
...
function scaleDeltaNearBounds(value,delta){
  if(delta>0)return delta*Math.max(.1,(100-value)/100);
  if(delta<0)return delta*Math.max(.1,value/100);
  return 0;
}
function scaleEmotionalDelta(e,kind,delta,{exceptional=false}={}){
  let applied=scaleDeltaNearBounds(kind==="morale"?Number(e.morale)||0:Number(e.stress)||0,Number(delta)||0);
  if(kind==="morale"&&applied>0&&!exceptional&&Number(e.morale)>85)applied*=Math.max(.12,(95-Number(e.morale))/10);
  const p=e.personality||{};
  const strength=clamp(1+(p.resilience||0)*-.08+(p.empathy||0)*.06+(p.structureNeed||0)*.04,.82,1.18);
  return Number((applied*strength).toFixed(4));
}
function recordEmotionalTrace(trace){
  if(!company)return trace;
  if(trace)trace.ownerSystem=AI_SYSTEM_OWNERS.emotional;
  company.emotionalTraces=Array.isArray(company.emotionalTraces)?company.emotionalTraces:[];
  company.emotionalTraces.unshift(trace);
  company.emotionalTraces=company.emotionalTraces.slice(0,180);
  return trace;
}
function applyEmployeeEmotionDelta(e,{moraleDelta=0,stressDelta=0,reasonCode="emotion",sourceEventId=null,relatedEmployeeIds=[],exceptional=false,ignoreCooldown=false,timestamp=null}={}){
  ...
  if(!ignoreCooldown&&e.emotionalCooldowns[code]>0){
    return recordEmotionalTrace({type:"emotional_event",employeeId:e.id,oldStress,stressDeltaRequested:requestedStress,stressDeltaScaled:0,stressDeltaApplied:0,newStress:oldStress,oldMorale,moraleDeltaRequested:requestedMorale,moraleDeltaScaled:0,moraleDeltaApplied:0,newMorale:oldMorale,reasonCode,sourceEventId,relatedEmployeeIds,blockedByCooldown:true,blockedByCap:false,status:"blocked_by_cooldown",timestamp:ts});
  }
  if(requestedStress===0&&requestedMorale===0){
    return recordEmotionalTrace({type:"emotional_event",employeeId:e.id,oldStress,stressDeltaRequested:0,stressDeltaScaled:0,stressDeltaApplied:0,newStress:oldStress,oldMorale,moraleDeltaRequested:0,moraleDeltaScaled:0,moraleDeltaApplied:0,newMorale:oldMorale,reasonCode,sourceEventId,relatedEmployeeIds,blockedByCooldown:false,blockedByCap:false,status:"zero_reaction",timestamp:ts});
  }
  const moraleScaled=scaleEmotionalDelta(e,"morale",requestedMorale,{exceptional});
  const stressScaled=scaleEmotionalDelta(e,"stress",requestedStress,{exceptional});
  const moraleApplied=emotionalCapDelta(e,"morale",moraleScaled),stressApplied=emotionalCapDelta(e,"stress",stressScaled);
  ...
  if(!ignoreCooldown)e.emotionalCooldowns[code]=1;
  e.lastEmotionalReaction={ownerSystem:AI_SYSTEM_OWNERS.emotional,moraleDelta:moraleApplied,stressDelta:stressApplied,reasonCode,sourceEventId,relatedEmployeeIds};
  e.recentEmotionalEvents=[{ownerSystem:AI_SYSTEM_OWNERS.emotional,day:company.day,minute:company.minute,reasonCode,moraleDelta:moraleApplied,stressDelta:stressApplied,sourceEventId},...(e.recentEmotionalEvents||[])].slice(0,12);
  ...
  const trace={ownerSystem:AI_SYSTEM_OWNERS.emotional,type:"emotional_event",employeeId:e.id,oldStress,stressDeltaRequested:requestedStress,stressDeltaScaled:stressScaled,stressDeltaApplied:stressApplied,newStress:e.stress,oldMorale,moraleDeltaRequested:requestedMorale,moraleDeltaScaled:moraleScaled,moraleDeltaApplied:moraleApplied,newMorale:e.morale,reasonCode,sourceEventId,relatedEmployeeIds,blockedByCooldown:false,blockedByCap,status:blockedByCap&&stressApplied===0&&moraleApplied===0?"blocked_by_daily_cap":blockedByCap?"applied_with_cap":"applied",timestamp:ts};
...
function applyEmotionalHomeostasis(e,{timestamp=null}={}){
  if(!e?.active)return null;
  const profile=ensureEmployeeEmotionalProfile(e);
  const ts=timestamp||simulationTimestamp();
  const oldMorale=Number(e.morale)||0,oldStress=Number(e.stress)||0;
...
function logSocialEmotionalTrace(trace){
  if(!company)return trace;
  const socialTrace=trace?{...trace,ownerSystem:AI_SYSTEM_OWNERS.social}:trace;
  company.socialEmotionTraces=Array.isArray(company.socialEmotionTraces)?company.socialEmotionTraces:[];
  company.socialEmotionTraces=[socialTrace,...company.socialEmotionTraces].slice(0,SOCIAL_RULES.maxEmotionTraces);
  company.lastSocialEmotionalEffect=socialTrace;
  return socialTrace;
}
...
function applyEmotionalRecommendation({employeeId,stressDelta=0,moraleDelta=0,reasonCode="emotion",sourceEventId=null,relatedEmployeeIds=[],timestamp=null}={}){
  ...
  const requestedStress=Number(stressDelta)||0,requestedMorale=Number(moraleDelta)||0,ts=timestamp||simulationTimestamp();
  if(!e){
    return logSocialEmotionalTrace({ownerSystem:AI_SYSTEM_OWNERS.social,type:"social_emotion_applied",employeeId:Number(employeeId),oldStress:null,stressDeltaRequested:requestedStress,stressDeltaApplied:0,newStress:null,oldMorale:null,moraleDeltaRequested:requestedMorale,moraleDeltaApplied:0,newMorale:null,reasonCode,sourceEventId,relatedEmployeeIds,blockedByCooldown:false,blockedByCap:false,status:"employee_missing",timestamp:ts});
  }
  const emotionalTrace=applyEmployeeEmotionDelta(e,{stressDelta:requestedStress,moraleDelta:requestedMorale,reasonCode,sourceEventId,relatedEmployeeIds,timestamp:ts});
  if(!emotionalTrace)return null;
  const socialTrace={...emotionalTrace,type:"social_emotion_applied",emotionalOwnerSystem:emotionalTrace.ownerSystem};
...
function simulationTimestamp(day=company?.day||0,minute=company?.minute||0){
  return {day,minute,absoluteMinute:day*TIME_RULES.minutesPerDay+minute};
}
function emptyRelationshipInterpretation(){
  return {...EMPTY_RELATIONSHIP_INTERPRETATION};
}
function normalizeRelationshipInterpretation(value={}){
  const neutral=emptyRelationshipInterpretation();
  return {
    trust:clamp(Number.isFinite(Number(value.trust))?Number(value.trust):neutral.trust,SCORE_DEFAULTS.minScore,SCORE_DEFAULTS.maxScore),
    respect:clamp(Number.isFinite(Number(value.respect))?Number(value.respect):neutral.respect,SCORE_DEFAULTS.minScore,SCORE_DEFAULTS.maxScore),
    comfort:clamp(Number.isFinite(Number(value.comfort))?Number(value.comfort):neutral.comfort,SCORE_DEFAULTS.minScore,SCORE_DEFAULTS.maxScore),
    professionalFriction:clamp(Number.isFinite(Number(value.professionalFriction))?Number(value.professionalFriction):neutral.professionalFriction,SCORE_DEFAULTS.minScore,SCORE_DEFAULTS.maxScore),
    confidence:clamp(Number.isFinite(Number(value.confidence))?Number(value.confidence):neutral.confidence,SCORE_DEFAULTS.minScore,SCORE_DEFAULTS.maxScore)
  };
}
function canonicalMigratedSocialExperienceType(type){
  const raw=String(type||"");
  if(SOCIAL_RULES.passiveLegacyExperienceTypes.includes(raw))return null;
  if(SOCIAL_EXPERIENCE_TYPES.has(raw))return raw;
...
function normalizeMigratedExperienceSummary(summary,recentExperiences=[]){
  ...
  if(summary&&typeof summary==="object"){
    Object.entries(summary).forEach(([rawType,item])=>{
      const type=canonicalMigratedSocialExperienceType(rawType);
      if(!type||!item||typeof item!=="object")return;
      const target=normalized[type]||{count:0,positive:0,neutral:0,negative:0,mixed:0,lastAt:null};
      for(const key of ["count","positive","neutral","negative","mixed"])target[key]+=Math.max(0,Number(item[key])||0);
      target.lastAt=Math.max(Number(target.lastAt)||0,Number(item.lastAt)||0)||null;
      normalized[type]=target;
    });
  }
  recentExperiences.forEach(experience=>{
    if(normalized[experience.type])return;
    const tone=["positive","neutral","negative","mixed"].includes(experience.emotionalTone)?experience.emotionalTone:"neutral";
    normalized[experience.type]={count:1,positive:0,neutral:0,negative:0,mixed:0,lastAt:experience.timestamp?.absoluteMinute??null};
    normalized[experience.type][tone]=1;
  });
  return normalized;
}
function normalizeSocialRelationshipRecord(record,key,{deriveInterpretation=true}={}){
  const ids=key.split(":").map(Number);
  if(ids.length!==2||ids.some(id=>!Number.isFinite(id)))return null;
  const legacyInterpretation=record?.interpretation||record?.relationship||null;
  record=record&&typeof record==="object"?record:{};
  record.employeeAId=Number.isFinite(Number(record.employeeAId))?Number(record.employeeAId):ids[0];
  record.employeeBId=Number.isFinite(Number(record.employeeBId))?Number(record.employeeBId):ids[1];
  record.familiarity=clamp(Number(record.familiarity)||0,SOCIAL_RULES.minFamiliarity,SOCIAL_RULES.maxFamiliarity);
  record.interactionCount=Math.max(0,Number(record.interactionCount)||0);
  record.firstMetAt=record.firstMetAt||null;
  record.lastInteractionAt=record.lastInteractionAt||null;
  record.lastSeenAt=record.lastSeenAt||null;
  record.coPresenceMinutes=Math.max(0,Number(record.coPresenceMinutes)||0);
  ...
  record.recentExperiences=Array.isArray(record.recentExperiences)
    ?record.recentExperiences.map(normalizeMigratedSocialExperience).filter(Boolean).slice(0,SOCIAL_RULES.maxRecentExperiences)
    :[];
  record.experienceSummary=normalizeMigratedExperienceSummary(record.experienceSummary,record.recentExperiences);
  record.positiveExperienceCount=Object.values(record.experienceSummary).reduce((sum,item)=>sum+Math.max(0,Number(item?.positive)||0),0);
  record.negativeExperienceCount=Object.values(record.experienceSummary).reduce((sum,item)=>sum+Math.max(0,Number(item?.negative)||0),0);
  record.neutralExperienceCount=Object.values(record.experienceSummary).reduce((sum,item)=>sum+Math.max(0,Number(item?.neutral)||0)+Math.max(0,Number(item?.mixed)||0),0);
  record.interpretation=normalizeRelationshipInterpretation(legacyInterpretation||{});
  delete record.relationship;
  ...
  record.recentInteractionTypes=Array.isArray(record.recentInteractionTypes)?record.recentInteractionTypes.map(item=>{
    const type=canonicalMigratedSocialExperienceType(item?.type);
    return type&&item?.sourceEventId?{...item,type,ownerSystem:AI_SYSTEM_OWNERS.social}:null;
  }).filter(Boolean).slice(0,SOCIAL_RULES.maxRecentInteractionTypes):[];
  record.reputationObservations=Array.isArray(record.reputationObservations)?record.reputationObservations.map(item=>{
    const type=canonicalMigratedSocialExperienceType(item?.type);
    return type&&(item?.sourceEventId||item?.sourceId)?{...item,type,ownerSystem:AI_SYSTEM_OWNERS.social}:null;
  }).filter(Boolean).slice(0,SOCIAL_RULES.maxReputationObservations):[];
  const preservedInteractionCount=Math.max(
    record.interactionCount,
    record.recentInteractionTypes.length,
    Object.values(record.experienceSummary).reduce((sum,item)=>sum+Math.max(0,Number(item?.count)||0),0)
  );
  ...
  const evidenceCount=record.positiveExperienceCount+record.neutralExperienceCount+record.negativeExperienceCount+
    Object.values(record.experienceSummary).reduce((sum,item)=>sum+Math.max(0,Number(item?.count)||0),0);
  if(evidenceCount>0&&deriveInterpretation)evaluateRelationshipInterpretation(record,{skipEnsure:true,timestamp:record.lastRelationshipEvaluationAt||record.lastInteractionAt||record.lastSeenAt||simulationTimestamp()});
  else if(evidenceCount>0){
    record.interpretation=normalizeRelationshipInterpretation(record.interpretation);
    record.relationshipInputs=record.relationshipInputs&&typeof record.relationshipInputs==="object"?record.relationshipInputs:{};
  }
  else{
    record.interpretation=emptyRelationshipInterpretation();
    record.relationshipInputs={};
    record.lastRelationshipEvaluationAt=null;
  }
  return record;
}
function ensureSocialAISystems({forceNormalize=false}={}){
  if(!company)return;
  const fromVersion=Number(company.socialAIModelVersion)||0;
  const targetVersion=SOCIAL_RULES.modelVersion;
  const rngStateBefore=Number(company.randomState)>>>0;
  ...
  company.socialPreferenceHistory=Array.isArray(company.socialPreferenceHistory)?company.socialPreferenceHistory:[];
  company.socialPreferenceDebug=Array.isArray(company.socialPreferenceDebug)?company.socialPreferenceDebug:[];
  company.socialEmotionTraces=Array.isArray(company.socialEmotionTraces)?company.socialEmotionTraces:[];
  company.emotionalTraces=Array.isArray(company.emotionalTraces)?company.emotionalTraces:[];
  ...
  const needsNormalization=forceNormalize||socialAIValidatedCompany!==company||fromVersion!==targetVersion;
  if(!needsNormalization)return;
  const migrationRequired=forceNormalize||fromVersion!==targetVersion;
  company.socialPreferenceHistory=company.socialPreferenceHistory.slice(0,SOCIAL_RULES.maxPreferenceHistory).map(trace=>({...trace,ownerSystem:AI_SYSTEM_OWNERS.social}));
  company.socialPreferenceDebug=company.socialPreferenceDebug.slice(0,SOCIAL_RULES.maxPreferenceDebugEntries).map(trace=>({...trace,ownerSystem:AI_SYSTEM_OWNERS.social}));
  company.socialEmotionTraces=company.socialEmotionTraces.slice(0,SOCIAL_RULES.maxEmotionTraces).map(trace=>({...trace,ownerSystem:AI_SYSTEM_OWNERS.social}));
  company.emotionalTraces=company.emotionalTraces.slice(0,180).map(trace=>({...trace,ownerSystem:AI_SYSTEM_OWNERS.emotional}));
  let relationshipsMigrated=0,legacyRecordsRemoved=0,experiencesPreserved=0;
  Object.entries(company.socialRelationships).forEach(([key,record])=>{
    if(record&&Object.prototype.hasOwnProperty.call(record,"relationship"))legacyRecordsRemoved++;
    const normalized=normalizeSocialRelationshipRecord(record,key,{deriveInterpretation:migrationRequired});
    if(!normalized){delete company.socialRelationships[key];legacyRecordsRemoved++;return;}
    company.socialRelationships[key]=normalized;
    relationshipsMigrated++;
    experiencesPreserved+=normalized.recentExperiences.length;
  });
  (employees||[]).forEach(e=>{
    if(Object.prototype.hasOwnProperty.call(e,"relationship")){delete e.relationship;legacyRecordsRemoved++;}
    if(Object.prototype.hasOwnProperty.call(e,"social")){delete e.social;legacyRecordsRemoved++;}
    if(e.goals)e.goals=normalizeEmployeeGoals(e.goals);
  });
  Object.entries(company.workplaceReputations).forEach(([id,rep])=>{
    ...
    rep.observations=Array.isArray(rep.observations)?rep.observations.slice(0,SOCIAL_RULES.maxReputationObservations):[];
    rep.recentChanges=Array.isArray(rep.recentChanges)?rep.recentChanges.slice(0,SOCIAL_RULES.maxReputationChanges):[];
    rep.confidence=clamp(Number(rep.confidence)||0,0,100);
    rep.lastUpdatedAt=rep.lastUpdatedAt||null;
  });
  company.socialMemories=company.socialMemories.slice(0,SOCIAL_RULES.maxSocialMemories).map(memory=>({...memory,ownerSystem:AI_SYSTEM_OWNERS.social}));
  company.socialAIModelVersion=targetVersion;
  if(migrationRequired||!company.socialAIMigrationReport){
    company.socialAIMigrationReport={
      ownerSystem:AI_SYSTEM_OWNERS.social,
      fromVersion,
      toVersion:targetVersion,
      relationshipsMigrated,
      legacyRecordsRemoved,
      experiencesPreserved,
      rngStateBefore,
      rngStateAfter:Number(company.randomState)>>>0
    };
  }
...
function workplaceReputationFor(employeeOrId,{create=false}={}){
  ...
  if(!company.workplaceReputations[id]&&create){
    company.workplaceReputations[id]={ownerSystem:AI_SYSTEM_OWNERS.social,employeeId:id,dimensions:{technical:50,reliability:50,approachability:50,professionalism:50},confidence:0,observations:[],recentChanges:[],lastUpdatedAt:null};
  }
...
function recordWorkplaceReputationObservation(employee,context={}){
  if(!context.sourceEventId&&!context.experienceId)return null;
  ...
  if(!type)return null;
  const sourceId=context.sourceEventId||context.experienceId;
  ...
  const observation={ownerSystem:AI_SYSTEM_OWNERS.social,day:company.day,minute:company.minute,type,tone,intensity,sourceId,delta};
  rep.observations=[observation,...(rep.observations||[])].slice(0,SOCIAL_RULES.maxReputationObservations);
  ...
  if(changed.length)rep.recentChanges=[{ownerSystem:AI_SYSTEM_OWNERS.social,day:company.day,minute:company.minute,type,tone,changes:changed},...(rep.recentChanges||[])].slice(0,SOCIAL_RULES.maxReputationChanges);
  return rep;
}
...
function socialRelationshipRecord(aId,bId,{create=false}={}){
  ...
  if(!company.socialRelationships[key]&&create){
    ...
    company.socialRelationships[key]={employeeAId,employeeBId,familiarity:0,interactionCount:0,firstMetAt:null,lastInteractionAt:null,lastSeenAt:null,coPresenceMinutes:0,positiveExperienceCount:0,neutralExperienceCount:0,negativeExperienceCount:0,recentExperiences:[],experienceSummary:{},interpretation:emptyRelationshipInterpretation(),relationshipInputs:{},lastRelationshipEvaluationAt:null,recentInteractionTypes:[],reputationObservations:[],cooldowns:{},stressHistory:0,moraleHistory:0};
  }
  return company.socialRelationships[key]||null;
}
function getRelationshipView(employeeAId,employeeBId){
  const aId=Number(employeeAId?.id??employeeAId),bId=Number(employeeBId?.id??employeeBId);
  const record=socialRelationshipRecord(aId,bId,{create:false});
  if(!record)return {employeeAId:aId,employeeBId:bId,familiarity:0,interactionCount:0,firstMetAt:null,lastInteractionAt:null,lastSeenAt:null,interpretation:emptyRelationshipInterpretation()};
  return {
    employeeAId:record.employeeAId,
    employeeBId:record.employeeBId,
    familiarity:Number(record.familiarity)||0,
    interactionCount:Number(record.interactionCount)||0,
    firstMetAt:record.firstMetAt||null,
    lastInteractionAt:record.lastInteractionAt||null,
    lastSeenAt:record.lastSeenAt||null,
    interpretation:normalizeRelationshipInterpretation(record.interpretation)
  };
}
function relationshipPreferenceScore(employeeAId,employeeBId){
  const view=getRelationshipView(employeeAId,employeeBId),rel=view.interpretation;
  const confidence=clamp(Number(rel.confidence)||0,0,100)/100;
  const interpreted=((rel.trust-50)*.32+(rel.respect-50)*.30+(rel.comfort-50)*.24-rel.professionalFriction*.28)*(.25+confidence*.75);
  return clamp(interpreted+Math.min(12,view.familiarity*.12),-100,100);
}
...
function evaluateRelationshipInterpretation(record,{skipEnsure=false,timestamp=null}={}){
  if(!skipEnsure)ensureSocialAISystems();
  ...
  if(!a||!b)return record.interpretation||emptyRelationshipInterpretation();
  ensureEmployeePersonality(a);ensureEmployeePersonality(b);
  const compat=personalityCompatibility(a,b),familiarity=clamp(Number(record.familiarity)||0,SOCIAL_RULES.minFamiliarity,SOCIAL_RULES.maxFamiliarity);
  const positive=Number(record.positiveExperienceCount)||0,neutral=Number(record.neutralExperienceCount)||0,negative=Number(record.negativeExperienceCount)||0,evidenceCount=positive+neutral+negative,total=Math.max(1,evidenceCount);
  if(evidenceCount<=0){
    record.interpretation=emptyRelationshipInterpretation();
    record.relationshipInputs={};
    record.lastRelationshipEvaluationAt=null;
...
function relationshipInterpretationFor(a,b){
  return getRelationshipView(a,b).interpretation;
}
function relationshipEmotionalReaction(employee,coworker,eventType="social"){
  ...
  const rel=record.interpretation||emptyRelationshipInterpretation();
...
function normalizeSocialExperienceType(type){
  ...
  const alias=SOCIAL_RULES.experienceAliases[type];
  return SOCIAL_EXPERIENCE_TYPES.has(alias)?alias:null;
}
...
function recordSocialInteractionEvidence(a,b,record,{type,sourceEventId,roomId=null,gain=1}={}){
  if(!record||!sourceEventId)return record;
  if((record.recentInteractionTypes||[]).some(x=>x.sourceEventId===sourceEventId&&x.type===type))return record;
  const now=simulationTimestamp(),first=record.interactionCount===0;
  record.familiarity=clamp((record.familiarity||0)+clamp(Number(gain)||0,0,5),SOCIAL_RULES.minFamiliarity,SOCIAL_RULES.maxFamiliarity);
  record.interactionCount++;
  if(first){
...
function socialPreferenceSocialBattery(employee){
  ...
  if(Number.isFinite(Number(state.socialBattery)))return clamp(Number(state.socialBattery),SCORE_DEFAULTS.minScore,SCORE_DEFAULTS.maxScore);
  return clamp(SCORE_DEFAULTS.healthyScore+(p.sociability||0)*22-(employee?.stress||SCORE_DEFAULTS.neutralScore)*.22+(employee?.morale||SCORE_DEFAULTS.neutralScore)*.12,SCORE_DEFAULTS.minScore,SCORE_DEFAULTS.maxScore);
}
...
function socialPreferenceWeight(employee,candidate,opportunity={}){
  ...
  const coworker=candidate,view=getRelationshipView(employee.id,coworker.id),rel=view.interpretation;
  const familiarity=Number(view.familiarity)||0,trust=Number(rel.trust)||0,respect=Number(rel.respect)||0,comfort=Number(rel.comfort)||0,friction=Number(rel.professionalFriction)||0;
  ...
  if(typeof conversationPartnerRecencyPenalty==="function")weight-=conversationPartnerRecencyPenalty(employee.id,coworker.id);
...
function chooseSocialPreference(employee,opportunity={},options={}){
  ...
  const decision={ownerSystem:AI_SYSTEM_OWNERS.social,day:company.day,minute:company.minute,employeeId:employee?.id,employeeName:employee?.name,opportunityType:opportunity.type||"conversation",roomId:room,selectedEmployeeId:selected?.employeeId??null,selectedName:selected?.name||"No interaction",selectedType:selected?.type||"none",reasonCode:selected?.reasonCode||"social_preference_none",candidates:weighted};
  if(options.record!==false){
    company.socialPreferenceHistory=[decision,...(company.socialPreferenceHistory||[])].slice(0,SOCIAL_RULES.maxPreferenceHistory);
    company.socialPreferenceDebug=[decision,...(company.socialPreferenceDebug||[])].slice(0,SOCIAL_RULES.maxPreferenceDebugEntries);
  }
  return decision;
}
function socialPreferenceEmotion(employee,preference,opportunity={}){
  ...
  if(selectedId==null){
    ...
    const coworker=(employees||[]).find(x=>x.id===selectedId),rel=getRelationshipView(employee.id,selectedId).interpretation;
    const trust=Number(rel.trust)||0,comfort=Number(rel.comfort)||0,friction=Number(rel.professionalFriction)||0,respect=Number(rel.respect)||0;
...
function recordSocialEncounter(a,b,{
  ...
  const last=Number(record.cooldowns[cooldownKey])||OFFICE_AQUARIUM_CONSTANTS.time.neverAbsoluteMinute;
...
function observeRoomFamiliarity(minutes=5){
  ...
  for(let i=0;i<present.length;i++)for(let j=i+1;j<present.length;j++){
    ...
    const a=present[i].employee,b=present[j].employee,roomId=present[i].room,key=`${makeRelationshipKey(a.id,b.id)}:${roomId}`;
    if(typeof maybeRecordOnboardingMentorInteraction==="function")maybeRecordOnboardingMentorInteraction(a,b,roomId);
    if(typeof maybeCreateAutomaticConflictRepair==="function")maybeCreateAutomaticConflictRepair(a,b,roomId);
    seen.add(key);
    company.roomPresenceCounters[key]=(company.roomPresenceCounters[key]||0)+minutes;
    const threshold=roomId==="break-area"?SOCIAL_RULES.passiveBreakThresholdMinutes:SOCIAL_RULES.passiveRoomThresholdMinutes;
    if(company.roomPresenceCounters[key]>=threshold){
      recordFamiliarityObservation(a,b,{roomId,minutes:company.roomPresenceCounters[key],gain:roomId==="break-area"?SOCIAL_RULES.passiveBreakFamiliarityGain:SOCIAL_RULES.passiveRoomFamiliarityGain,cooldownMinutes:roomId==="break-area"?90:120});
...
function familiaritySummaryForEmployee(e){
  ...
  const known=records.filter(r=>r.familiarity>=SOCIAL_RULES.familiarityKnownThreshold).length,well=records.filter(r=>r.familiarity>=60).length;
...
function socialFamiliarityDebugHtml(e){
  ...
  return rows.map(([key,r])=>{
    ...
    const rel=r.interpretation||emptyRelationshipInterpretation(),inputs=r.relationshipInputs||{},last=r.lastRelationshipEvaluationAt?`day ${r.lastRelationshipEvaluationAt.day}, minute ${r.lastRelationshipEvaluationAt.minute}`:"not evaluated";
    return `Pair ${key} (${other?.name||"missing employee"})<br>Familiarity ${Number(r.familiarity||0).toFixed(2)}; observed together ${Math.round(r.coPresenceMinutes||0)} minute(s); real interactions ${r.interactionCount||0}<br>First interaction ${r.firstMetAt?`day ${r.firstMetAt.day}, minute ${r.firstMetAt.minute}`:"not recorded"}; last interaction ${r.lastInteractionAt?`day ${r.lastInteractionAt.day}, minute ${r.lastInteractionAt.minute}`:"none"}; last seen ${r.lastSeenAt?`day ${r.lastSeenAt.day}, minute ${r.lastSeenAt.minute}`:"none"}<br>Relationship Interpretation: Trust ${rel.trust||0}; Respect ${rel.respect||0}; Comfort ${rel.comfort||0}; Professional Friction ${rel.professionalFriction||0}; Confidence ${rel.confidence||0}<br>Interpretation Inputs familiarity ${inputs.familiarity??"n/a"}; compatibility ${inputs.compatibility??"n/a"}; style mismatch ${inputs.styleMismatch??"n/a"}; recent help ${inputs.recentHelp??"n/a"}; recent friction ${inputs.recentFriction??"n/a"}; last evaluation ${last}<br>Recent ${recent}<br>Shared Experiences ${experiences}<br>Aggregate Counts +${r.positiveExperienceCount||0} / ${(r.neutralExperienceCount||0)} / -${r.negativeExperienceCount||0}<br>Stress history ${Number(r.stressHistory||0).toFixed(2)}; morale history ${Number(r.moraleHistory||0).toFixed(2)}<br>Cooldowns ${Object.keys(r.cooldowns||{}).length}`;
  }).join("<br><br>");
}
...
{id:"culture",repeatable:true,title:"Culture conflict reaches leadership",copy:"A disagreement over priorities is spreading through the office.",trigger:()=>lowestMorale()<42,choices:[{title:"Back management",detail:"Clarify authority but risk trust.",effect:{board:3,trust:-2},directive:"speed",days:8,people:{stress:7,morale:-6}},{title:"Back the engineers",detail:"Improve morale but weaken authority.",effect:{board:-3,trust:2},directive:"quality",days:10,people:{stress:-5,morale:9}},{title:"Order mediation",detail:"Repair relationships and reduce stress.",effect:{cash:-.5,trust:1},directive:"people",days:12,people:{stress:-9,morale:6}}]},
...
function normalizeEmployeeGoals(goals={}){
  const normalized={mastery:.5,promotion:.5,socialConnection:.5,stability:.5,recognition:.5,...goals};
  const legacyKey="friend"+"ship",legacyValue=normalized[legacyKey];
  if(Number.isFinite(Number(legacyValue))&&!Number.isFinite(Number(goals.socialConnection)))normalized.socialConnection=Number(legacyValue);
  delete normalized[legacyKey];
  return normalized;
}
function makeEmployee(i){
  const goalProfiles=[
    {mastery:.92,promotion:.72,socialConnection:.28,stability:.45,recognition:.62},
    {mastery:.66,promotion:.48,socialConnection:.76,stability:.42,recognition:.55},
    {mastery:.88,promotion:.58,socialConnection:.24,stability:.52,recognition:.46},
    {mastery:.84,promotion:.38,socialConnection:.36,stability:.78,recognition:.42},
    {mastery:.54,promotion:.82,socialConnection:.72,stability:.38,recognition:.84},
    {mastery:.82,promotion:.46,socialConnection:.34,stability:.64,recognition:.52},
    {mastery:.74,promotion:.56,socialConnection:.58,stability:.48,recognition:.68},
    {mastery:.62,promotion:.64,socialConnection:.32,stability:.82,recognition:.58}
  ];
  ...
  const employee=normalizeEmployeeRoleProfile({
    ...
    focus:55+simulationRandom()*30,goals:normalizeEmployeeGoals(goalProfile),
    ...
    motion:null,conversationPresence:null,officeFlow:null,
    lastAction:null,repeatCount:0,cooldowns:{break:0,meeting:0,socialize:0,collaborate:0,complain:0},
    decisionTrace:{ownerSystem:AI_SYSTEM_OWNERS.work,chosen:"arriving",scores:{},reasons:[]},opinionOfCEO:{trust:62,fairness:58,competence:64,support:56,fear:12},careerLevel:1,careerHistory:["Founding team"],beliefs:{},dailyBriefing:null,currentIntention:null,skills:baseSkillsForRole(role),performance:{recentOutput:0,absenceDays:0,qualityMistakes:0,coachingDays:0,reviewRiskDays:0,lastReviewDay:OFFICE_AQUARIUM_CONSTANTS.time.neverDay},learning:{caution:0,mentor:0,risk:0,collaboration:0,helpSeeking:0,testing:0,focusWork:0,reporting:0,suppression:0,initiative:0,recovery:0,contextualPreferences:{}},communication:{reportsMade:0,reportsSuppressed:0,helpRequests:0,lastReportDay:OFFICE_AQUARIUM_CONSTANTS.time.neverDay,lastHelpRequestDay:OFFICE_AQUARIUM_CONSTANTS.time.neverDay,rumorsShared:0},knownMessages:[],actionOutcomeContext:null,activeCollaboration:null,activeMeeting:null,learnedLessons:{testing:0,collaboration:0,documentation:0,escalation:0,innovation:0,riskTaking:0,planning:0,mentoring:0,recovery:0},lessonAcceptance:null,joinedDay:company?.day||0,age:28+i*4+Math.floor(simulationRandom()*4),stayScore:72,retentionRisk:28,jobSearchDays:0,retirementReadiness:0,quarterlyReview:null,promotionExpectation:45,salarySatisfaction:65,recognitionSatisfaction:60
  });
...
function reset(skipSave=false,randomize=false,seedState=null){document.getElementById("gameOverOverlay")?.classList.add("hidden");document.getElementById("runtimeErrorOverlay")?.classList.add("hidden");company=typeof structuredClone==="function"?structuredClone(initialCompany):JSON.parse(JSON.stringify(initialCompany));resetCommunicationUi();if(Number.isFinite(seedState))company.randomState=seedState>>>0;if(randomize){const climates=[{name:"Cautious market",cash:-1.5,board:4,trust:3},{name:"Investor optimism",cash:2,board:6,trust:-2},{name:"Competitive pressure",cash:0,board:-3,trust:1},{name:"Stable opening",cash:0,board:0,trust:0}];const climate=climates[Math.floor(simulationRandom()*climates.length)];company.cash=clamp(company.cash+climate.cash,14,21);company.board=clamp(company.board+climate.board,58,82);company.trust=clamp(company.trust+climate.trust,55,78);company.valuation=clamp(company.valuation+rand(-4,4),34,50);company.log=[`You became CEO during a ${climate.name.toLowerCase()}. The office is running without direct employee control.`];}employees=names.map((_,i)=>makeEmployee(i));if(randomize){const shuffled=traits.map(x=>[...x]).sort(()=>simulationRandom()-.5);employees.forEach((e,i)=>{e.traits=shuffled[i];e.energy=clamp(e.energy+rand(-8,8),55,95);e.morale=clamp(e.morale+rand(-8,8),52,90);e.focus=clamp(e.focus+rand(-8,8),45,92);});}company.weekStartSnapshot=captureWeekSnapshot();ensureBibleSystems?.();ensureProjectPortfolio?.();updateManufacturingAndStakeholders?.();updateCompanyInformationSystem?.();collectDailyMetrics?.();buildOffice();renderDecisionEvent();render();if(!validationMode)restartTimer();if(!skipSave&&!validationMode)saveGame();}
function saveStatusCopy(result,error){
  if(error){
    if(error.code==="QUOTA_EXCEEDED")return "Autosave stopped: this device is out of local storage.";
    if(error.code==="SAVE_TOO_LARGE")return "Autosave stopped: this company needs save recovery.";
    return "Autosave unavailable. Your last valid save is still protected.";
  }
  if(result?.sizeStatus==="compaction")return "Saved - storage use is high";
  if(result?.sizeStatus==="warning")return "Saved - storage use is increasing";
  return "Saved on this device";
}
function saveGame(options={}){
  try{
    if(currentSimulationContext?.mode==="isolated-validation")throw new SaveRepositoryError("VALIDATION_SAVE_BLOCKED","Production save blocked during isolated validation.");
    const result=saveRepository.write(company,employees);
    company.saveHealth={status:result.sizeStatus,lastSuccessfulDay:company.day,lastSuccessfulSize:result.size,backupAvailable:result.backupCreated||saveRepository.readDetailed().recoverable,errorCode:null,emergency:!!options.emergency};
    const saveNote=document.getElementById("saveNote");if(saveNote)saveNote.textContent=saveStatusCopy(result,null);
    renderSaveRecoveryNotice?.();
    return {ok:true,...result};
  }catch(error){
    company.saveHealth={...(company.saveHealth||{}),status:"error",errorCode:error.code||"SAVE_FAILED",lastFailureDay:company.day,emergency:!!options.emergency};
    const saveNote=document.getElementById("saveNote");if(saveNote)saveNote.textContent=saveStatusCopy(null,error);
    renderSaveRecoveryNotice?.();
    return {ok:false,errorCode:error.code||"SAVE_FAILED",message:saveStatusCopy(null,error)};
  }
}
function loadGame(){try{const data=saveRepository.read();if(!data)return false;lastLoadFailure=null;company={...saveRepository.clone(initialCompany),...(data.company||{})};const loadedRandomState=Number.isFinite(company.randomState)?(company.randomState>>>0):OFFICE_AQUARIUM_CONSTANTS.determinism.defaultRandomState;const loadedNextRuntimeId=Math.max(1,Number(company.nextRuntimeId)||1);company.randomState=loadedRandomState;company.nextRuntimeId=loadedNextRuntimeId;const migrationEmployeeDefaults=i=>{const savedRandom=company.randomState;company.randomState=(0x9e3779b9+Math.imul(i+1,0x85ebca6b))>>>0;const fresh=makeEmployee(i);company.randomState=savedRandom;return fresh;};const savedEmployees=Array.isArray(data.employees)?data.employees:[];const employeeSlots=Math.max(names.length,...savedEmployees.map(e=>Number(e.id)).filter(Number.isFinite).map(id=>id+1));employees=Array.from({length:employeeSlots},(_,i)=>{const fresh=migrationEmployeeDefaults(i),saved=savedEmployees.find(e=>e.id===i)||{};return{
      ...fresh,...saved,
      goals:normalizeEmployeeGoals({...fresh.goals,...(saved.goals||{})}),
      ...
      emotionalProfile:saved.emotionalProfile&&typeof saved.emotionalProfile==="object"?saved.emotionalProfile:fresh.emotionalProfile,
    ...
    };});company.randomState=loadedRandomState;employees.forEach(e=>{normalizeEmployeeRoleProfile(e);ensureEmployeePersonality(e);e.goals=normalizeEmployeeGoals(e.goals);delete e.relationship;delete e.social;});company.completedEvents=Array.isArray(company.completedEvents)?company.completedEvents:[];company.costEfficiency=clamp(Number(company.costEfficiency)||1,.72,1.08);company.pilotDays=Number(company.pilotDays)||0;
...
company.communicationArchiveMode=company.communicationArchiveMode==="deleted"?"deleted":"saved";
company.selectedArchiveMessageIndex=Number.isInteger(company.selectedArchiveMessageIndex)?company.selectedArchiveMessageIndex:null;
company.pendingCommunication=company.pendingEvent?(company.pendingCommunication||null):null;
company.randomState=Number.isFinite(company.randomState)?(company.randomState>>>0):OFFICE_AQUARIUM_CONSTANTS.determinism.defaultRandomState;
company.nextRuntimeId=Math.max(1,Number(company.nextRuntimeId)||1);
company.saveHealth={...initialCompany.saveHealth,...(company.saveHealth||{})};
company.runtimeFailure=company.runtimeFailure&&typeof company.runtimeFailure==="object"?company.runtimeFailure:null;
...
company.lastLessonReviewDay=Number.isFinite(company.lastLessonReviewDay)?company.lastLessonReviewDay:OFFICE_AQUARIUM_CONSTANTS.time.neverDay;
...
company.playtest={sessions:0,notes:[],lastChecklistDay:OFFICE_AQUARIUM_CONSTANTS.time.neverDay,...(company.playtest||{})};
company.employeeMessages=Array.isArray(company.employeeMessages)?company.employeeMessages:[];
company.emotionalTraces=Array.isArray(company.emotionalTraces)?company.emotionalTraces.slice(0,180):[];
...
company.lastInformationalEscalationDay=Number.isFinite(Number(company.lastInformationalEscalationDay))?Number(company.lastInformationalEscalationDay):OFFICE_AQUARIUM_CONSTANTS.time.neverDay;
company.lastDecisionEscalationDay=Number.isFinite(Number(company.lastDecisionEscalationDay))?Number(company.lastDecisionEscalationDay):OFFICE_AQUARIUM_CONSTANTS.time.neverDay;
company.decisionEscalationFingerprints=company.decisionEscalationFingerprints&&typeof company.decisionEscalationFingerprints==="object"?company.decisionEscalationFingerprints:{};
company.lastHiringRequestMemoDay=Number.isFinite(Number(company.lastHiringRequestMemoDay))?Number(company.lastHiringRequestMemoDay):OFFICE_AQUARIUM_CONSTANTS.time.neverDay;
...
company.randomState=loadedRandomState;company.nextRuntimeId=loadedNextRuntimeId;ensureBibleSystems?.();ensureProjectPortfolio?.();rebuildRuntimeIndexes?.();ensureProjectAllocations?.();company.randomState=loadedRandomState;company.nextRuntimeId=loadedNextRuntimeId;buildOffice(true);renderDecisionEvent();render();restartTimer();company.randomState=loadedRandomState;company.nextRuntimeId=loadedNextRuntimeId;lastLoadFailure=null;return true;}catch(e){lastLoadFailure={code:"MIGRATION_FAILED",message:"This saved company could not be updated safely."};if(debugMode)console.warn("Save migration failed",e);return false;}}

function hasSavedCompany(){return saveRepository.exists();}
function savedCompanySummary(){return saveRepository.summary();}
function saveRecoveryDetail(){return saveRepository.readDetailed();}
function updateStartupScreen(){
  const detail=saveRecoveryDetail(),info=savedCompanySummary(),btn=document.getElementById("continueCompany"),summary=document.getElementById("saveSummary");
  const recovery=document.getElementById("saveRecoveryPanel"),recoveryCopy=document.getElementById("saveRecoveryCopy"),restore=document.getElementById("restoreBackup"),exportCurrent=document.getElementById("exportCurrentSave"),exportBackup=document.getElementById("exportBackupSave");
  document.querySelectorAll("[data-release-version]").forEach(element=>{element.textContent=OFFICE_AQUARIUM_CONSTANTS.release.displayVersion;});
  const currentLoadable=["valid","migration-required"].includes(detail.current.status);
  if(btn)btn.disabled=!currentLoadable||!!lastLoadFailure;
  if(restore)restore.disabled=!detail.recoverable;
  if(exportCurrent)exportCurrent.disabled=!detail.current.raw;
  if(exportBackup)exportBackup.disabled=!detail.backup.raw;
  const needsRecovery=!!lastLoadFailure||["corrupt-current-backup-available","backup-only","corrupt-no-backup","storage-unavailable"].includes(detail.status);
  recovery?.classList.toggle("hidden",!needsRecovery);
  if(recoveryCopy){
    recoveryCopy.textContent=lastLoadFailure
      ?"The saved company could not be updated safely. Restore the protected backup if one is available, or export the save before starting again."
      :detail.recoverable
      ?"A protected backup is available. Restore it to continue, or export either save before making changes."
      :detail.status==="storage-unavailable"
        ?"Local storage is unavailable. You can cancel and keep this page open, or start a new temporary company."
        :"No valid backup was found. Export the damaged save before starting a new company if you want to keep it for support.";
  }
  if(!summary)return;
  if(lastLoadFailure){summary.textContent="The saved company could not be updated to this release. Your stored data has not been replaced.";return;}
  if(detail.status==="storage-unavailable"){summary.textContent="This browser is not allowing local saves. You can start a temporary company, but progress may not be preserved.";return;}
  if(detail.status==="corrupt-current-backup-available"){summary.textContent="The current save could not be read, but a valid last-known-good backup is available.";return;}
  if(detail.status==="backup-only"){summary.textContent="The main save is missing, but a valid last-known-good backup is available.";return;}
  if(detail.status==="corrupt-no-backup"){summary.textContent="The saved company is damaged and no valid backup was found. You can export the raw save before starting again.";return;}
  if(!info){summary.textContent="No saved company was found on this device.";return;}
  const when=info.savedAt?new Date(info.savedAt).toLocaleString():"an earlier session";
  const migration=detail.status==="migration-required"?" It will be updated safely when opened.":"";
  summary.textContent=`Saved company: Day ${info.day}, ${info.identity}, $${info.cash.toFixed(1)}M cash, $${info.valuation.toFixed(1)}M valuation - saved ${when}.${migration}`;
}
function downloadLocalText(filename,text,mime="application/json"){
...
function exportSaveSlot(slot="current"){
  try{
    const raw=saveRepository.exportRaw(slot);
    if(!raw)return false;
    return downloadLocalText(
      `office-aquarium-${slot}-save.${OFFICE_AQUARIUM_CONSTANTS.release.saveExportExtension}`,
      raw,
      "application/octet-stream"
    );
  }catch(error){return false;}
}
function restoreBackupCompany(){
  try{
    saveRepository.restoreBackup();
    lastLoadFailure=null;
    updateStartupScreen();
    if(loadGame()){enterSimulation();return true;}
  }catch(error){}
  updateStartupScreen();
  return false;
}
function dismissSaveRecovery(){document.getElementById("saveRecoveryPanel")?.classList.add("hidden");}
function openStartupHandbook(){
  handbookOpenedFromStartup=true;
  document.getElementById("startupOverlay")?.classList.add("hidden");
  openSimulationHandbook?.();
}
function returnFromStartupHandbook(){
  if(!handbookOpenedFromStartup)return;
  handbookOpenedFromStartup=false;
...
function renderSaveRecoveryNotice(){
  ...
  if(!banner)return;
  const health=company.saveHealth||{},status=health.status;
  const visible=["warning","compaction","error"].includes(status);
  banner.classList.toggle("hidden",!visible);
  banner.classList.toggle("error",status==="error");
  if(!visible)return;
  if(status==="error"){
    banner.textContent=health.errorCode==="QUOTA_EXCEEDED"
      ?"Autosave has stopped because this device is out of local storage. Your last valid save and backup are still protected."
      :"Autosave could not finish. Your last valid save is still protected; return to the title screen for recovery options.";
  }else{
    banner.textContent=status==="compaction"
      ?"This company is using a large amount of local save storage. Office Aquarium is compacting repeated history while preserving outcomes."
      :"This company is growing. Local save storage is being monitored and remains available.";
  }
}
function playerSafeRuntimeStage(phase){
  const raw=String(phase||"simulation"),stage=raw.includes(":")?raw.split(":").pop():raw;
  return OFFICE_AQUARIUM_CONSTANTS.runtimeRecovery.safeStageLabels[stage]||"the company simulation";
}
function runtimeDiagnosticsPayload(){
  return {
    product:"Office Aquarium",
    applicationVersion:OFFICE_AQUARIUM_CONSTANTS.release.applicationVersion,
    saveVersion:SAVE_VERSION,
    exportedAt:new Date().toISOString(),
    company:{day:company?.day,minute:company?.minute,phase:company?.phase,employees:employees?.filter(e=>e.active).length,projects:company?.projects?.length||0},
    runtimeFailure:company?.runtimeFailure||null,
    lastSimulationError:company?.lastSimulationError||null,
    lastDailyCloseStatus:company?.lastDailyCloseStatus||null,
    systemErrors:(company?.systemErrors||[]).slice(0,OFFICE_AQUARIUM_CONSTANTS.runtimeRecovery.maximumSystemErrors),
    saveHealth:company?.saveHealth||null,
    userAgent:typeof navigator!=="undefined"?navigator.userAgent:"unavailable"
  };
}
function exportRuntimeDiagnostics(){
  return downloadLocalText(`office-aquarium-diagnostics-day-${(company?.day||0)+1}.json`,JSON.stringify(runtimeDiagnosticsPayload(),null,2));
}
function showRuntimeFailure(errorRecord,saveResult=null){
  if(!company)return;
  const record=errorRecord||company.lastSimulationError||{};
  const prior=company.runtimeFailure&&company.runtimeFailure.phase===record.phase?company.runtimeFailure:null;
  company.runtimeFailure={
    day:company.day,
    minute:company.minute,
    phase:record.phase||"simulation",
    safeStage:playerSafeRuntimeStage(record.phase),
    saveStatus:saveResult?.ok
      ?"Emergency save verified"
      :prior?.saveStatus||(company.saveHealth?.backupAvailable
        ?"Emergency save failed; last-known-good backup available"
        :"Emergency save failed; keep the game open and export diagnostics")
  };
  if(typeof document==="undefined"||validationMode)return;
  const overlay=document.getElementById("runtimeErrorOverlay");
  const stage=document.getElementById("runtimeErrorStage"),saveStatus=document.getElementById("runtimeErrorSaveStatus"),restore=document.getElementById("runtimeRestoreBackup");
  if(stage)stage.textContent=company.runtimeFailure.safeStage;
  if(saveStatus)saveStatus.textContent=company.runtimeFailure.saveStatus;
  if(restore)restore.disabled=!saveRepository.readDetailed().recoverable;
  overlay?.classList.remove("hidden");
  document.getElementById("runtimeErrorTitle")?.focus();
}
function restoreBackupAfterRuntimeError(){
  try{
    saveRepository.restoreBackup();
...
function enterSimulation(){document.getElementById("startupOverlay").classList.add("hidden");document.getElementById("saveRecoveryPanel")?.classList.add("hidden");syncSoundUi();startBackgroundMusic();}
function startNewCompany(){lastLoadFailure=null;saveRepository.remove();reset(false,true);enterSimulation();document.getElementById("confirmResetOverlay").classList.add("hidden");}
function returnToTitle(){if(company){company.paused=true;updatePauseButton();}stopBackgroundMusic();document.getElementById("runtimeErrorOverlay")?.classList.add("hidden");document.getElementById("startupOverlay")?.classList.remove("hidden");updateStartupScreen();}
...
function performanceTarget(){
  return employees.filter(e=>e.active).map(e=>{
    e.performance={recentOutput:0,absenceDays:0,qualityMistakes:0,coachingDays:0,reviewRiskDays:0,lastReviewDay:OFFICE_AQUARIUM_CONSTANTS.time.neverDay,...(e.performance||{})};
    return {e,score:performanceScore(e),riskDays:e.performance.reviewRiskDays||0,lastReviewDay:e.performance.lastReviewDay??OFFICE_AQUARIUM_CONSTANTS.time.neverDay};
...
function availableCollaborator(e){
  ...
  return employees
    .filter(x=>{
      if(!x.active||x.offsite||x.id===e.id||x.stress>=88||x.action==="leading a meeting")return false;
      const work=typeof activeWorkForEmployee==="function"?activeWorkForEmployee(x):null;
      const deadlineNear=work&&Number.isFinite(Number(work.deadlineDay))&&Number(work.deadlineDay)-company.day<=OFFICE_AQUARIUM_CONSTANTS.social.conversations.criticalDeadlineWindowDays;
      const critical=work&&(deadlineNear||(work.blockedBy||[]).length||(Number(work.priority)||0)>=OFFICE_AQUARIUM_CONSTANTS.social.conversations.criticalPriorityThreshold);
      return !(critical&&["working","testing hardware"].includes(x.action));
    })
    .map(x=>({x,score:roleCompatibility(e,x,need)+(x.focus||50)*.12+(x.morale||50)*.05-(x.stress||0)*.18+(x.action?.includes("collaborating")?-8:0)}))
...
function employeeMovementDurationMs(e,targetX,targetY){
  const distance=Math.hypot((Number(targetX)||0)-(Number(e?.x)||0),(Number(targetY)||0)-(Number(e?.y)||0));
  return Math.round(clamp(
    OFFICE_AQUARIUM_CONSTANTS.social.conversations.movementBaseDurationMs+distance*OFFICE_AQUARIUM_CONSTANTS.social.conversations.movementPerDistanceMs,
    OFFICE_AQUARIUM_CONSTANTS.social.conversations.movementBaseDurationMs,
    OFFICE_AQUARIUM_CONSTANTS.social.conversations.movementMaxDurationMs
  ));
}
function moveEmployeeToPosition(e,targetX,targetY,{instant=false,reason="activity-transition",flowCue=null}={}){
  if(!e)return;
  const fromX=Number.isFinite(Number(e.x))?Number(e.x):Number(targetX)||50;
  const fromY=Number.isFinite(Number(e.y))?Number(e.y):Number(targetY)||50;
  const x=clamp(Number(targetX)||fromX,1,99),y=clamp(Number(targetY)||fromY,1,99);
  const durationMs=instant?0:employeeMovementDurationMs(e,x,y);
  e.motion={
    fromX,
    fromY,
    targetX:x,
    targetY:y,
    startedAt:typeof simulationTimestamp==="function"?simulationTimestamp():null,
    durationMs,
    easing:"accelerate-decelerate",
    reason,
    flowCue:flowCue||null
  };
  e.officeFlow=flowCue?{cue:flowCue,startedAt:e.motion.startedAt}:e.officeFlow||null;
  e.x=x;e.y=y;
  if(typeof document==="undefined")return;
  const node=document.getElementById(`agent-${e.id}`);
  if(!node)return;
  if(instant)node.style.transition="none";
  else node.style.setProperty("--agent-move-duration",`${durationMs}ms`);
  node.style.left=`${x}%`;
  node.style.top=`${y}%`;
  if(instant)setTimeout(()=>node.style.transition="",20);
}
function moveToZone(e,z,instant=false){
  const previousRoom=e?.currentRoom||roomForZone(e?.zone);
  const zone=zones[z]||zones[e?.homeZone]||zones.studio;
  e.zone=zones[z]?z:e?.homeZone||"studio";
  const nextRoom=roomForZone(e.zone);
  e.currentRoom=nextRoom||e.currentRoom||null;
  const targetX=rand(zone.x[0],zone.x[1]),targetY=rand(zone.y[0],zone.y[1]);
  if(!instant&&e.conversationPresence){
    e.officeFlow={
      ...(e.officeFlow||{}),
      cue:"resume-activity",
      startedAt:typeof simulationTimestamp==="function"?simulationTimestamp():null,
      pendingPosition:{x:targetX,y:targetY,zone:e.zone,currentRoom:e.currentRoom}
    };
  }else{
    moveEmployeeToPosition(e,targetX,targetY,{instant,reason:"zone-transition",flowCue:instant?null:"resume-activity"});
  }
  if(!instant&&previousRoom&&nextRoom&&previousRoom!==nextRoom&&nextRoom===e.homeRoom&&OFFICE_AQUARIUM_CONSTANTS.rooms.departmentWorkRooms.includes(previousRoom)&&typeof createGroundedConversationOpportunity==="function"){
    const partner=employees
      .filter(candidate=>candidate.active&&!candidate.offsite&&candidate.id!==e.id&&(candidate.currentRoom||roomForZone(candidate.zone))===nextRoom)
      .sort((a,b)=>relationshipPreferenceScore(e,b)-relationshipPreferenceScore(e,a)||a.id-b.id)[0];
    if(partner){
      const work=typeof activeWorkForEmployee==="function"?activeWorkForEmployee(e):null;
      createGroundedConversationOpportunity({
        id:`department-return-${e.id}-${previousRoom}-${nextRoom}-${company.day}-${Math.floor(company.minute/30)}`,
        ...
        context:{
          ...
          projectId:work?.projectId||null,
          purpose:`returning from ${teamDisplayName(previousRoom)||previousRoom}`
        }
      });
    }
  }
}
function socialTarget(e){return employees.filter(x=>x.active&&!x.offsite&&x.id!==e.id).sort((a,b)=>relationshipPreferenceScore(e,b)-relationshipPreferenceScore(e,a))[0]||null;}
