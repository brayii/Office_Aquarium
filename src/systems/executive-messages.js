...
function ensureExecutiveMessageModel(ev,comm){
  ...
  model.messageType=String(comm.type||"Executive Email").includes("Board")?"Board Memorandum":comm.priority==="FYI"?"Informational Update":"Executive Email";
  ...
  model.decisionRequired=comm.priority!=="FYI";
...
function projectPortfolioHtml(){
  ...
  const projectCard=p=>{
    ...
    const overall=projectVisibleHealth(p);
    ...
    const required=Object.values(p.requiredHeadcount||{}).reduce((a,b)=>a+(Number(b)||0),0),missing=projectStaffing.filter(x=>x.project.id===p.id),missingText=missing.length?missing.map(x=>`+${x.missing} ${x.role} (${teamDisplayName(x.dept)})`).join("; "):"covered or under review";
    return `<div class="briefing-card"><strong>${p.title}</strong><small>${p.family} | ${p.originType} | ${projectStatusLabel(p)}${lineage}<br>Overall Health: ${overall}<br>Progress ${Math.round(p.progress||0)}%; target ${p.estimatedDuration||"?"} day(s); pace ${paceLabel}; spend ${spend}<br>Timing: ${timing.short}; ${timing.status}; ${timing.confidenceLabel} confidence<br>Schedule ${perf.scheduleVariance??0}; budget ${perf.budgetVariance??0}; quality ${Math.round(perf.quality||p.quality||0)}<br>Required staff ${required}; staffing ${Math.round(perf.staffingCoverage??derivedStaffingCoverage(p))}; overload ${Math.round(perf.workloadOverload||projectOverloadPressure(p))}; missing ${missingText}<br>Customer interest ${Math.round(perf.customerInterest||p.customerInterest||0)}; risk ${Math.round(perf.riskTrend||p.visibleRisk||0)}; confidence ${Math.round(perf.strategicConfidence||p.visibleConfidence||0)}<br>Commercial: ${commercialLabel}; readiness ${Math.round(p.commercialReadiness||0)}; potential ${Math.round(p.commercialPotential||0)}; ${commercialRevenueText}; customers ${Math.round(p.convertedCustomers||0)}<br>Recommendation: ${rec}</small><div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:8px">${actions}</div></div>`;
  };
...
function recordThreadFollowUp(thread,phase,text,{memo=false,priority="FYI",actionRequired=false}={}){
  ...
  if(memo){
    company.communications=Array.isArray(company.communications)?company.communications:[];
    company.communications.unshift({id:nextSimulationId("followup"),eventId:thread.eventId,title:`Follow-up: ${thread.subject}`,type:"Follow-up",priority,from:thread.from,role:thread.role,date:`Day ${company.day+1}`,message:text,recs:[],impacts:[`Original decision: ${thread.decision}`],signature:`Regards,\n${thread.from}\n${thread.role}`,decision:"No action required",decisionDetail:"Follow-up memo only; no new CEO decision was requested.",decisionStrategy:thread.strategy,uncertainty:"Observed",estimatedConfidence:65,benefits:[],risks:[],day:company.day,memoIntelligence:{department:thread.department,evidence:[`Original decision: ${thread.decision}`,text],recommendation:"No action required.",noActionForecast:"The decision thread will continue to be monitored.",uncertainty:"This is an observed follow-up, not a new request.",alternativeRecommendations:[],chainOfCommand:[`${thread.from} followed up on the original CEO decision.`]},memoAudit:null,laterOutcome:null,forecastAccuracy:null,credibilityUpdated:false,decisionThreadId:thread.id});
    company.communications=company.communications.slice(0,60);
  }
}
...
function processDecisionThreads(){
  ...
  company.decisionThreads.forEach(thread=>{
    if(!thread.audit.reaction&&company.day>=thread.phaseDueDays.reaction){
      ...
      employees.filter(e=>e.active&&employeeTeam(e)===thread.department).forEach(e=>{e.morale=clamp(e.morale+(thread.expectedTone==="positive"?1:thread.expectedTone==="negative"?-1:0),0,100);e.stress=clamp(e.stress+(thread.expectedTone==="negative"?1:0),0,100);});
...
function processDelayedDecisionEffects(){
  ...
  due.forEach(x=>{
    ...
    employees.filter(e=>e.active).forEach(e=>{
      e.stress=clamp(e.stress+(x.people?.stress||0),0,100);
      e.morale=clamp(e.morale+(x.people?.morale||0),0,100);
    });
    clampCompany();
    ...
    recordWeeklyEvent(`Decision outcome: ${x.message}`,"leadership",x.tone==="negative"?4:3);
...
learnFromDecision(ev,d);if(!ev.repeatable&&!company.completedEvents.includes(ev.id))company.completedEvents.push(ev.id);company.log.push(`CEO decision: ${d.title}. A longer-term outcome is expected around day ${delayedOutcome.dueDay+1}.`);recordWeeklyEvent(`CEO decision: ${d.title}.`,"leadership",4);recordHistory(`CEO decision: ${d.title}.`,"leadership",4);if(thread)recordHistory(`Decision thread opened for "${d.title}" from ${comm.from}.`,"leadership",3);company.pendingEvent=null;company.pendingCommunication=null;company.eventCooldown=720;company.paused=false;if(!validationMode){document.getElementById("pauseBtn").textContent="Pause";saveGame();renderDecisionEvent();render();}}
...
function resolvePerformance(mode){
  ...
  if(mode==="coach"){
    ...
    target.focus=clamp(target.focus+14,0,100);target.morale=clamp(target.morale+12,0,100);target.stress=clamp(target.stress-10,0,100);target.performance={recentOutput:0,absenceDays:0,qualityMistakes:0,coachingDays:0,reviewRiskDays:0,lastReviewDay:-999,...(target.performance||{})};target.performance.coachingDays=12;
    ...
    target.focus=clamp(target.focus+7,0,100);target.stress=clamp(target.stress+5,0,100);company.integration+=2;company.quality+=2;
...
function resolveHiring(mode,roleOverride=null){
  ...
  const role=roleOverride||company.openRoles[0];
  if(!role)return;
  ...
  if(mode==="specialist"){
    ...
    if(candidate){
      ...
      candidate.morale=clamp(candidate.morale+8,0,100);
      candidate.stress=clamp(candidate.stress+12,0,100);
