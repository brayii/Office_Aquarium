...
function applyDailyOrganizationalPressure(morale,stress){
  ...
  active.forEach(e=>{
    ...
    applyEmployeeEmotionDelta(e,{stressDelta:personal*2.6,moraleDelta:-personal*2.2-(e.morale>88?Math.min(.45,(e.morale-88)*.035):0),reasonCode:"organizational-pressure",sourceEventId:`org-pressure-${company.day}`,ignoreCooldown:true});
...
function maybeQueueBoardPortfolioConcern(){
  ...
  company.escalationQueue.push({id:`portfolio-board-review-${company.day}`,repeatable:false,category:"project",title:"Board portfolio review",copy:`The board flagged ${concerns.join(", ")}.`,generatedCommunication:{type:"Board Portfolio Letter",priority:"Decision Needed",sender:{name:"Board Strategy Committee",role:"Board"},subject:"Portfolio Discipline Review",message:`The board is asking for a portfolio direction because ${concerns.join(", ")}. This is not a request to assign individual tasks. It is a request to decide whether the company should review risk, keep backing growth work, or reduce project spending.`,recs:[["Board","Review portfolio discipline before risk spreads",82],["Finance",h.totalProjectSpendDaily>.12?"Reduce active project spending":"Keep spending tied to milestones",72],["Portfolio","Use gated reviews and cancellation discipline",76]],impacts:["Board confidence may change","Project reviews may be accelerated","Hiring and restructuring pressure may follow"]},choices:[{title:"Review the riskiest projects",detail:"Queue reviews for the projects most likely to miss schedule, budget, or staffing expectations.",effect:{board:1},directive:"quality",days:7,portfolioAction:"triage",strategy:"risk-control",benefits:["forces evidence review"],risks:["slows momentum"],uncertainty:"Material",estimatedConfidence:68},{title:"Continue funding growth projects",detail:"Keep the highest-upside projects moving even though portfolio risk is visible.",effect:{board:-2,trust:1},directive:"speed",days:8,portfolioAction:"protect-growth",strategy:"growth",benefits:["preserves upside"],risks:["board patience falls"],uncertainty:"High",estimatedConfidence:48},{title:"Reduce active project spending",detail:"Narrow scope or pace across the active portfolio to protect runway and focus.",effect:{board:3},directive:"cuts",days:8,portfolioAction:"cut-burn",strategy:"cost-control",benefits:["improves runway"],risks:["morale and opportunity loss"],uncertainty:"Material",estimatedConfidence:58}]});
}
...
function choicesForProjectControl(p,requested="review"){
  ...
  const base={
    ...
    expand:canExpand?{title:"Increase project budget",detail:"Fund a larger push while the window is open.",effect:{cash:-Math.min(1.2,p.estimatedCost*.18),board:2},directive:"speed",days:8,projectDecision:{id:p.id,action:"expand"},strategy:"growth",benefits:["raises upside","accelerates learning"],risks:["budget and staffing pressure rise"],uncertainty:"High",estimatedConfidence:52}:null,
...
function applyProjectDecision(decision){
  ...
  else if(decision.action==="reject"){p.status="rejected";p.closedDay=company.day;archiveProjectOnce(p,"rejected");employees.filter(e=>e.active&&e.projectAffinity?.[p.id]).forEach(e=>{applyEmployeeEmotionDelta(e,{moraleDelta:-3,reasonCode:"project-rejected",sourceEventId:p.id,ignoreCooldown:true});adjustCEOOpinion(e,{support:-2,trust:-1});});recordHistory(`${p.title} was rejected during portfolio review.`,"project",4);}
