...
function makeBoardValuationMemo(kind,view){
  ...
  const base={id:`valuation-${kind}-${company.day}`,repeatable:false,category:"board",title:"Investor confidence and valuation review",copy:"Investor Relations summarized market reaction, and the Board interpreted it through company performance and Board experience.",generatedCommunication:{type:"Board Market Letter",priority:view.interpretation==="overheated"||view.interpretation==="weakening"?"Urgent":"Decision Needed",sender:{name:"Board Market Committee",role:"Board"},subject:"Investor Confidence and Market Perception Review",message:`Investor Relations reports ${report.confidenceTrend} investor confidence with ${concern}. The Board view is that ${view.boardPosition}. The company is viewed as a ${companyIdentityLabel().toLowerCase()} with ${valuationQualityLabel()} valuation support. Thirty-day valuation movement is ${view.trend.toFixed(1)}%, investor confidence is ${Math.round(view.investorSignal)}, market confidence is ${Math.round(company.marketConfidence)}, and fundamentals are ${Math.round(view.fundamentals)}.`,recs:[["Board",view.interpretation==="overheated"?"Use the window carefully; support is fragile":view.interpretation==="weakening"?"Show disciplined execution before patience worsens":view.interpretation==="mispriced"?"Do not let a noisy market force a bad operating decision":"Keep market actions tied to evidence",78],["Investor Relations",opportunity!=="no clear investor opportunity"?`Investors are most receptive to ${opportunity}`:`Current investor questions focus on ${concern}`,72],["Finance",company.cash<8?"Consider funding if terms are acceptable":"Avoid raising money just because valuation moved",70],["People",company.marketConfidence>62?"A durable reputation can support recruiting":"Unstable perception can hurt candidates and retention",62]],impacts:[...(report.evidence||[]).slice(0,3),"Valuation may move without cash changing","Fundraising changes ownership and expectations","Board confidence depends on valuation quality, not only valuation level"]},choices:[]};
